...
import routes from "@/constants/routes.json"; // Assuming routes.json contains the graph data
...
const RouteDetails = () => {
  ...
  for (let i = 0; i < stations.length - 1; i++) {
    ...
    if (routeData) {
      ...
      if (Array.isArray(routeData.line)) {
        ...
        const sharedLinesBoth = routeData.line.filter(
          (line) =>
            (nextRouteData?.line || []).includes(line) &&
            (previousRouteData?.line || []).includes(line)
        );
        const sharedLinesEither = routeData.line.filter(
          (line) =>
            (nextRouteData?.line || []).includes(line) ||
            (previousRouteData?.line || []).includes(line)
        );
        ...
        const expressRouteSameLine = routeData.line.find(
          (line) =>
        ...
        selectedRoute =
          expressRouteSameLine ||
          sharedLinesBoth[0] ||
          sharedLinesEither[0] ||
          (previousRoute && routeData.line.includes(previousRoute)
            ? previousRoute
            : routeData.line[0]);
  ...
  for (let i = 1; i < stations.length - 1; i++) {
    const currentStation = stations[i];
    const isRouteChange =
      routeNames[i - 1] && routeNames[i] && routeNames[i - 1] !== routeNames[i];
